...
function OnMind(props){
    ...
    useEffect(()=>{
        const getuser=async()=>{
             const response=await axios.get(`http://localhost:4300/veteran/${props.location.state.detail}`)
    ...
    const onc=()=>{
     ...
     const onc1=()=>{
        document.getElementById('selectedVid').click()
     }
    ...
    const onsubmit=async()=>{
       const response=axios.post('http://localhost:4300/veteran/post', object)
    ...
    <> 
     <div id="mindCont">
       ...
       <h3 style={{fontSize:'30px',marginLeft:'10px', fontWeight:'bold', marginTop:'30px'}} >{user.name}</h3>
        ...
        <input type="file" id="selectedVid" style={{display: 'none'}} onChange={showFile1} />
